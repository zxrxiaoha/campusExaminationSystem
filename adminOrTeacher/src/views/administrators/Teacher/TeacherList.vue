<template>
  <div>
    ...
    >
      ...
      <el-table-column label="编号" prop="tId" sortable> </el-table-column>
        ...
        <el-row>
          <el-col :span="12">
            ...
            <el-form-item label="性别：" prop="gender">
              <el-radio v-model="model.gender" label="1" disabled>男</el-radio>
              <el-radio v-model="model.gender" label="0" disabled>女</el-radio>
            </el-form-item>
            <el-form-item label="出生日期：">
              <el-date-picker
                v-model="model.introductionToThe.dateOfBirthStr"
                type="date"
                :clearable="false"
                readonly
                placeholder="值为空"
...
export default {
  ...
  methods: {
    ...
    async handleEdit(index, row) {
      this.$router.push({ path: '/admin_main/teacher/create', query: { id: row._id } })
    },
    handleDelete(row) {
      this.$confirm(`是否删除此条数据?`, '危险', {
        ...
        let success = await this.$http.delete(`rest/teacher_users/${row._id}`);
    ...
    async paging(page) {
      const res = await this.$http.get(`rest/teacher_users/${page}`);
      // console.log(res);
      this.tableData = res.data
    },
  },
