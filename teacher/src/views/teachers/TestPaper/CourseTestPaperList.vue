<template>
  <div>
    <!-- 保存的试卷列表 -->
    <el-table
      :data="searchResult"
      style="width: 100%"
      stripe
      border
      highlight-current-row
    >
      <el-table-column
        label="Date"
        prop="date"
        sortable
        width="100px"
        align="center"
      >
      </el-table-column>
      <el-table-column
        label="课程"
        prop="profession_course.name"
        width="200px"
        align="center"
      >
      </el-table-column>
      <el-table-column
        label="试卷标题"
        prop="title"
        width="200px"
        align="center"
      >
      </el-table-column>
      <el-table-column
        label="考试开始时间"
        prop="timeStr[0]"
        sortable
        width="200px"
        align="center"
      >
      </el-table-column>
      <el-table-column
        label="考试结束时间"
        prop="timeStr[1]"
        sortable
        width="200px"
        align="center"
      >
      </el-table-column>
      <el-table-column label="状态" prop="isEnd" width="110px" align="center">
        <template slot-scope="scope">
          <div :class="{zc:!scope.row.isEnd,gq:scope.row.isEnd}">{{scope.row.isEnd?'过期':'正常'}}</div>
        </template>
      </el-table-column>
      <el-table-column
        label="考试时长"
        prop="durationStr"
        sortable
        width="150px"
        align="center"
      >
      </el-table-column>
      <el-table-column
        label="试卷总分"
        prop="total"
        sortable
        width="150px"
        align="center"
      >
      </el-table-column>
      <el-table-column
        label="试卷题数"
        prop="number"
        sortable
        width="150px"
        align="center"
      >
      </el-table-column>
      <el-table-column
        label="单选题数"
        prop="singleTopicSelection.length"
        sortable
        width="150px"
        align="center"
      >
      </el-table-column>
      <el-table-column
        label="多选题数"
        prop="multipleChoice.length"
        sortable
        width="150px"
        align="center"
      >
      </el-table-column>
      <el-table-column
        label="判断题数"
        prop="trueFalse.length"
        sortable
        width="150px"
        align="center"
      >
      </el-table-column>
      <el-table-column align="right" fixed="right" width="240px">
        <template slot="header" slot-scope="scope">
          <el-input
            v-model="search"
            size="mini"
            placeholder="输入时间标题搜索"
          />
        </template>
        <template slot-scope="scope">
          <el-button
            size="mini"
            @click="publishedPapers(scope.$index, scope.row)"
            type="success"
            >{{ scope.row.isSave ? "已发布" : "发布" }}</el-button
          >
          <el-button
            size="mini"
            @click="handleEdit(scope.$index, scope.row)"
            type="primary"
            >编辑</el-button
          >
          <el-button size="mini" type="danger" @click="handleDelete(scope.row)"
            >删除</el-button
          >
        </template>
      </el-table-column>
    </el-table>
  </div>
</template>

<script>
export default {
  name: "TeacherList",
  data() {
    return {
      tableData: [],
      search: '',
    }
  },
  computed: {
    //搜索，toLowerCase：将字符串转为小写
    searchResult() {
      return this.tableData.filter(data => {
        return !this.search || data.name.toLowerCase().includes(this.search.toLowerCase());
      })
    },

  },
  methods: {
    //发布试卷，判断试卷是否已发布
    //发布试卷，判断试卷是否已发布
    async publishedPapers(index, row) {
      //还没发布过
      if (row.isSave == false) {
        //设置为发布过
        row.isSave = true;
        let res = await this.$http.put(`rest/course_test_paper/${row._id}`, row);//发请求
        if (res.status == 200 && res.data) {
          this.$message({
            type: 'success',
            message: '发布成功!'
          });
        }
      } else {
        this.$message({
          type: 'error',
          message: '该试卷已发布过了!'
        });
      }
    },
    async handleEdit(index, row) {
      this.$router.push({ path: '/teacher_main/course_test_paper/create', query: { id: row._id } })
    },
    handleDelete(row) {
      this.$confirm(`是否删除此条数据?`, '危险', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'error',
        center: true
      }).then(async () => {
        let success = await this.$http.delete(`rest/course_test_paper/${row._id}`);
        if (success.data.success) {
          this.$message({
            type: 'success',
            message: '删除成功!'
          });
        } else {
          this.$message({
            type: 'info',
            message: '删除失败!'
          });
        }
        this.fetch();
      }).catch(() => {
        this.$message({
          type: 'info',
          message: '已取消删除'
        });
      });
    },
    openFullScreen2() {
      this.loading = this.$loading({
        lock: true,
        text: 'Loading',
        spinner: 'el-icon-loading',
        background: 'rgba(0, 0, 0, 0.7)'
      });
    },
    async fetch() {
      let tId = localStorage.t_user ? JSON.parse(localStorage.t_user).tId : '';
      if (tId) {
        //该老师的课程试卷
        const res = await this.$http.get('rest/course_test_paper/' + tId);
        if (res.status == 200) {
          // this.loading.close()
        }
        this.tableData = res.data
      }
      // this.openFullScreen2();

    },
  },
  created() {
    this.fetch();
  },

}
</script>
<style  scoped>
  .zc{
    color: #67c23a;
    font-weight: bold;
  }
  .gq{
    color: red;
    font-weight: bold;
  }
</style>